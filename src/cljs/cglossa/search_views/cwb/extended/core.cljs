...
(defn- checkboxes [wrapped-query wrapped-term lemmatized? phon-display-name has-orig? first? last? speech?]
  (let [term-val @wrapped-term]
    [:div.table-cell {:style {:min-width 200}}
     [:div.word-checkboxes
      ...
      [:label.checkbox-inline {:style {:padding-left (if lemmatized? 26 18)}}
       [:input {:type      "checkbox"
                :style     {:margin-left -18}
                :title     "Start of word"
...
(defn multiword-term [a m wrapped-query wrapped-term query-term-ids
                      index first? last? lemmatized? phon-display-name has-orig? show-remove-row-btn?
  ...
  (r/with-let [show-attr-popup? (r/atom false)]
    [:div.table-cell>div.multiword-term>div.control-group
     ...
     [:div.table-row
      (when first?
        [:div.table-cell])
      [checkboxes wrapped-query wrapped-term lemmatized? phon-display-name has-orig? first? last? speech?]
      (when last?
        [:div.table-cell])]
                           ...
                           (let [wrapped-term          (r/wrap term
                             ...
                             (list (when-not first?
                                   ...
                                   [multiword-term a m wrapped-query wrapped-term query-term-ids
                                    index first? last? (has-lemma? @corpus) (get-phonetic @corpus)
                                    (has-original? @corpus) show-remove-row-btn?
                                    show-remove-term-btn? speech?])))
                         terms))]
